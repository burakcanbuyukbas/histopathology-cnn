...
train_generator, val_generator, test_generator = get_data_generators(image_size=image_size, batch_size=batch_size, model='resnet')
...
model = create_custom_model2(image_size)
...
model.save("model_custom2_" + str(image_size) + "px.h5")
try:
    plot_acc(history)
    plot_loss(history)
finally:
